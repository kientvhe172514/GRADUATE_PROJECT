...
export class RefreshTokenUseCase {
  ...
  async execute(refreshTokenDto: RefreshTokenRequestDto, ipAddress?: string, userAgent?: string): Promise<ApiResponseDto<RefreshTokenResponseDto>> {
    ...
    try {
      ...
      await this.logFailedRefresh(null, ipAddress, userAgent, 'Invalid token format');
    ...
    if (!account) {
      await this.logFailedRefresh(payload.sub, ipAddress, userAgent, 'Account not found');
      throw new UnauthorizedException('Account not found');
    }

    // Check if account is active
    if (account.status !== 'ACTIVE') {
      await this.logFailedRefresh(account.id!, ipAddress, userAgent, 'Account is not active');
    ...
    if (!refreshTokenRecord) {
      await this.logFailedRefresh(account.id!, ipAddress, userAgent, 'Refresh token not found');
    ...
    if (refreshTokenRecord.expires_at < new Date()) {
      ...
      await this.logFailedRefresh(account.id!, ipAddress, userAgent, 'Refresh token expired');
      throw new UnauthorizedException('Refresh token expired');
    }

    // Check if token is revoked
    if (refreshTokenRecord.revoked_at) {
      await this.logFailedRefresh(account.id!, ipAddress, userAgent, 'Refresh token has been revoked');
