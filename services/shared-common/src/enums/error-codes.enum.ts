export enum ErrorCodes {
  // General errors
  BAD_REQUEST = 'BAD_REQUEST',
  UNAUTHORIZED = 'UNAUTHORIZED',
  FORBIDDEN = 'FORBIDDEN',
  NOT_FOUND = 'NOT_FOUND',
  INTERNAL_SERVER_ERROR = 'INTERNAL_SERVER_ERROR',
  VALIDATION_ERROR = 'VALIDATION_ERROR',
  MISSING_REQUIRED_FIELDS = 'MISSING_REQUIRED_FIELDS',
  INVALID_DATE_RANGE = 'INVALID_DATE_RANGE',
  INVALID_STATE_TRANSITION = 'INVALID_STATE_TRANSITION',
  INVALID_INPUT = 'INVALID_INPUT',

  // Auth specific errors
  INVALID_CREDENTIALS = 'INVALID_CREDENTIALS',
  ACCOUNT_LOCKED = 'ACCOUNT_LOCKED',
  ACCOUNT_NOT_FOUND = 'ACCOUNT_NOT_FOUND',
  ACCOUNT_ALREADY_EXISTS = 'ACCOUNT_ALREADY_EXISTS',
  PASSWORD_TOO_WEAK = 'PASSWORD_TOO_WEAK',
  TEMPORARY_PASSWORD_MUST_CHANGE = 'TEMPORARY_PASSWORD_MUST_CHANGE',
  REFRESH_TOKEN_INVALID = 'REFRESH_TOKEN_INVALID',
  REFRESH_TOKEN_EXPIRED = 'REFRESH_TOKEN_EXPIRED',
  TOKEN_EXPIRED = 'TOKEN_EXPIRED',
  INVALID_TOKEN = 'INVALID_TOKEN',

  // Notification specific errors
  NOTIFICATION_NOT_FOUND = 'NOTIFICATION_NOT_FOUND',
  NOTIFICATION_ALREADY_READ = 'NOTIFICATION_ALREADY_READ',
  INVALID_NOTIFICATION_TYPE = 'INVALID_NOTIFICATION_TYPE',
  INVALID_DELIVERY_CHANNEL = 'INVALID_DELIVERY_CHANNEL',
  NOTIFICATION_SEND_FAILED = 'NOTIFICATION_SEND_FAILED',
  TEMPLATE_NOT_FOUND = 'TEMPLATE_NOT_FOUND',
  INVALID_TEMPLATE_VARIABLES = 'INVALID_TEMPLATE_VARIABLES',
  PREFERENCE_NOT_FOUND = 'PREFERENCE_NOT_FOUND',
  INVALID_PREFERENCE_SETTING = 'INVALID_PREFERENCE_SETTING',
  PUSH_TOKEN_NOT_FOUND = 'PUSH_TOKEN_NOT_FOUND',
  INVALID_PUSH_TOKEN = 'INVALID_PUSH_TOKEN',
  PUSH_TOKEN_REGISTRATION_FAILED = 'PUSH_TOKEN_REGISTRATION_FAILED',
  EMAIL_SERVICE_UNAVAILABLE = 'EMAIL_SERVICE_UNAVAILABLE',
  PUSH_SERVICE_UNAVAILABLE = 'PUSH_SERVICE_UNAVAILABLE',
  SMS_SERVICE_UNAVAILABLE = 'SMS_SERVICE_UNAVAILABLE',
  RATE_LIMIT_EXCEEDED = 'RATE_LIMIT_EXCEEDED',
  DO_NOT_DISTURB_ACTIVE = 'DO_NOT_DISTURB_ACTIVE',

  // Employee specific errors
  EMPLOYEE_ALREADY_EXISTS = 'EMPLOYEE_ALREADY_EXISTS',
  EMPLOYEE_NOT_FOUND = 'EMPLOYEE_NOT_FOUND',
  EMPLOYEE_CODE_ALREADY_EXISTS = 'EMPLOYEE_CODE_ALREADY_EXISTS',
  EMPLOYEE_EMAIL_ALREADY_EXISTS = 'EMPLOYEE_EMAIL_ALREADY_EXISTS',
  DEPARTMENT_NOT_FOUND = 'DEPARTMENT_NOT_FOUND',
  DEPARTMENT_CODE_ALREADY_EXISTS = 'DEPARTMENT_CODE_ALREADY_EXISTS',
  POSITION_NOT_FOUND = 'POSITION_NOT_FOUND',
  POSITION_CODE_ALREADY_EXISTS = 'POSITION_CODE_ALREADY_EXISTS',
  MANAGER_NOT_FOUND = 'MANAGER_NOT_FOUND',

  // RBAC & Permission errors
  ROLE_NOT_FOUND = 'ROLE_NOT_FOUND',
  ROLE_CODE_ALREADY_EXISTS = 'ROLE_CODE_ALREADY_EXISTS',
  PERMISSION_NOT_FOUND = 'PERMISSION_NOT_FOUND',
  PERMISSION_CODE_ALREADY_EXISTS = 'PERMISSION_CODE_ALREADY_EXISTS',
  PERMISSION_DENIED = 'PERMISSION_DENIED',
  INSUFFICIENT_PERMISSIONS = 'INSUFFICIENT_PERMISSIONS',

  // API Key errors
  API_KEY_NOT_FOUND = 'API_KEY_NOT_FOUND',
  API_KEY_INVALID = 'API_KEY_INVALID',
  API_KEY_EXPIRED = 'API_KEY_EXPIRED',
  API_KEY_IP_NOT_WHITELISTED = 'API_KEY_IP_NOT_WHITELISTED',
  API_KEY_RATE_LIMIT_EXCEEDED = 'API_KEY_RATE_LIMIT_EXCEEDED',

  // Leave Management errors
  LEAVE_TYPE_NOT_FOUND = 'LEAVE_TYPE_NOT_FOUND',
  LEAVE_TYPE_CODE_ALREADY_EXISTS = 'LEAVE_TYPE_CODE_ALREADY_EXISTS',
  LEAVE_BALANCE_NOT_FOUND = 'LEAVE_BALANCE_NOT_FOUND',
  LEAVE_RECORD_NOT_FOUND = 'LEAVE_RECORD_NOT_FOUND',
  INSUFFICIENT_LEAVE_BALANCE = 'INSUFFICIENT_LEAVE_BALANCE',
  LEAVE_REQUEST_OVERLAPS = 'LEAVE_REQUEST_OVERLAPS',
  INVALID_LEAVE_DATE_RANGE = 'INVALID_LEAVE_DATE_RANGE',
  LEAVE_ALREADY_APPROVED = 'LEAVE_ALREADY_APPROVED',
  LEAVE_ALREADY_REJECTED = 'LEAVE_ALREADY_REJECTED',
  LEAVE_CANNOT_BE_CANCELLED = 'LEAVE_CANNOT_BE_CANCELLED',

  // Holiday errors
  HOLIDAY_NOT_FOUND = 'HOLIDAY_NOT_FOUND',
  HOLIDAY_ALREADY_EXISTS = 'HOLIDAY_ALREADY_EXISTS',
  INVALID_HOLIDAY_DATE = 'INVALID_HOLIDAY_DATE',
  HOLIDAY_IN_USE = 'HOLIDAY_IN_USE',

  // Attendance & Scheduling errors
  SCHEDULE_NOT_FOUND = 'SCHEDULE_NOT_FOUND',
  SCHEDULE_NAME_ALREADY_EXISTS = 'SCHEDULE_NAME_ALREADY_EXISTS',
  SCHEDULE_NAME_REQUIRED = 'SCHEDULE_NAME_REQUIRED',
  INVALID_SCHEDULE_CONFIG = 'INVALID_SCHEDULE_CONFIG',
  SCHEDULE_ALREADY_ACTIVE = 'SCHEDULE_ALREADY_ACTIVE',
  SCHEDULE_ALREADY_INACTIVE = 'SCHEDULE_ALREADY_INACTIVE',
  SHIFT_ALREADY_STARTED = 'SHIFT_ALREADY_STARTED',
  SHIFT_NOT_IN_PROGRESS = 'SHIFT_NOT_IN_PROGRESS',
  CHECK_IN_MISSING = 'CHECK_IN_MISSING',
  REASON_REQUIRED = 'REASON_REQUIRED',
}
