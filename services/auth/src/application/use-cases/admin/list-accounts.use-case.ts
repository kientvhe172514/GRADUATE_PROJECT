...
export class ListAccountsUseCase {
  ...
  async execute(dto: ListAccountsRequestDto): Promise<ApiResponseDto<ListAccountsResponseDto>> {
    try {
      ...
      console.log('🔍 ListAccountsUseCase - Incoming DTO:', JSON.stringify(dto, null, 2));
      console.log('🔍 dto.status:', dto.status, 'type:', typeof dto.status);
      console.log('🔍 dto.role:', dto.role, 'type:', typeof dto.role);
      console.log('🔍 dto.department_id:', dto.department_id, 'type:', typeof dto.department_id);
      ...
      if (dto.department_id !== undefined && dto.department_id !== null) {
        ...
        console.log('✅ Department ID filter added:', searchCriteria.department_id);
      }
      ...
      console.log('🔍 Criteria passed to repository:', JSON.stringify(criteria, null, 2));
      
      const { accounts, total } = await this.accountRepo.findWithPagination(criteria);
