  ...
  async execute(
    ...
    try {
      ...
      if (!dto.status || !validStatuses.includes(receivedStatus)) {
        throw new BusinessException(
          ErrorCodes.BAD_REQUEST,
          'Invalid status. Must be one of: ACTIVE, INACTIVE, LOCKED, SUSPENDED',
        );
      }
      ...
      console.log(
        'UpdateAccountStatus: Current account status:',
        account.status,
      );
      console.log(
        'UpdateAccountStatus: Account status type:',
      ...
      if (account.status === receivedStatus) {
        throw new BusinessException(
          ErrorCodes.BAD_REQUEST,
          'Account already has this status',
        );
      }

      // Update account status
      const updatedAccount = await this.accountRepo.updateStatus(
        accountId,
        receivedStatus,
      );
